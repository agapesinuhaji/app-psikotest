...
<head>
    ...
    {{-- CSS ANIMASI BLOB (TIDAK DIBUTUHKAN JIKA MENGGUNAKAN FILE CSS EXTERNAL) --}}
    <style>
        /* Keyframes untuk animasi pergerakan blob */
        @keyframes blob-move {
...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex items-center justify-center min-h-screen relative overflow-hidden">
    ...
    {{-- 2. HEADER/NAVIGASI (Tetap Minimalis di Kanan Atas) --}}
    <header class="w-full max-w-7xl absolute top-0 pt-6 px-6 lg:pt-8 lg:px-8 z-20">
        @if (Route::has('login'))
            <nav class="flex items-center justify-end gap-4">
                @auth
                    <a href="{{ url('/dashboard') }}" class="px-5 py-1.5 text-sm font-medium border rounded-sm text-[#1b1b18] dark:text-[#EDEDEC] border-transparent hover:border-gray-300 dark:hover:border-[#3E3E3A]">Dashboard</a>
                @endauth
            </nav>
        @endif
    </header>

    {{-- 3. KONTEN UTAMA (Logo, Nama, Deskripsi, Tombol) --}}
    <main class="relative z-10 flex flex-col items-center justify-center p-8 text-center max-w-2xl bg-white/60 dark:bg-[#161615]/70 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/40 dark:border-black/40 transition-opacity duration-750 opacity-100 starting:opacity-0">
        
        {{-- Logo (Ganti dengan SVG atau Image Tag Anda) --}}
        <div class="mb-4 w-16 h-16 flex items-center justify-center rounded-full bg-blue-600/10 dark:bg-blue-300/10 p-2">
        ...
        {{-- Nama Perusahaan --}}
        <h1 class="mb-2 text-4xl lg:text-3xl font-extrabold tracking-tight text-[#1b1b18] dark:text-[#EDEDEC]">
            {{ config('app.name', 'PEXRA ANALYTICS') }}
        </h1>
        
        {{-- Teks Singkat --}}
        <p class="mb-10 text-lg lg:text-xl text-[#706f6c] dark:text-[#A1A09A] font-light max-w-md">
            Solusi terdepan untuk **manajemen data yang efisien** dan pengambilan keputusan strategis.
        </p>

        {{-- Tombol Login Utama (Menarik Perhatian) --}}
        @if (Route::has('login'))
            <a
                href="{{ route('login') }}"
                class="
                    ...
                    bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full shadow-xl
    ...
    {{-- Footer --}}
    <footer class="absolute bottom-0 text-xs text-[#706f6c] dark:text-[#A1A09A] py-4 z-10">
